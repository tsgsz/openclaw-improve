...
description: 监控特定 session 的状态，判断任务是否完成/失败/需要延长
triggers:
  - 当 runtime-monitor 检测到任务超时
  - 需要检查 session 实际状态
---
...
3. **做出判断**
   - **已完成**: 任务实际已完成，更新 runtime.json 标记完成
   - **失败**: 任务卡住或失败，通知 orchestrator
   - **需要延长**: 任务正常进行但需要更多时间，重新预估 ETA
...
4. **执行操作并通知 orchestrator**
   ...
   **所有情况都必须通知 orchestrator**:
   ...
   ```python
   # 情况1: 已完成
   os.system(f'python3 ~/.openclaw/workspace/scripts/runtime-monitor.py complete {runId}')
   使用 session_send 通知 orchestrator: "任务 {runId} 已完成"
   
   # 情况2: 需要延长
   os.system(f'python3 ~/.openclaw/workspace/scripts/runtime-monitor.py update-eta {runId} {newETA}')
